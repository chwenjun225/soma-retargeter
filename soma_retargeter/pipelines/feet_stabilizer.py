...
class FeetStabilizer:
    ...
    def __init__(self, config: str):
        ...
        self.target = get_target(self.robot_type)
        ...
        body_names = [
            newton_utils.get_name_from_label(label) for label in self.robot_builder.body_label]
        missing = sorted(set(self.effectors) - set(body_names))
        if missing:
            raise ValueError(
                f"[ERROR]: Feet stabilizer bodies do not exist on {self.robot_type}: {missing}")
        self.effector_mapped_indices = [
            body_names.index(body_name) for body_name in self.effectors]
        self.effector_weights = [
            wp.vec2(*tr_weights) for tr_weights in self.effectors.values()]
        effector_parent_indices = [
            self.robot_builder.joint_parent[idx] for idx in self.effector_mapped_indices]

        self.pelvis_idx = self.effector_mapped_indices[self.ik_root]
        self.two_bone_ik_chains = wp.array2d(
            [[self.effector_mapped_indices[i]
              for i in limb[_LIMB_DATA_IDX_EFFECTOR_INDICES]]
             for limb in self.ik_limb_data], dtype=wp.int32)
        self.two_bone_ik_chain_parent = wp.array(
            [effector_parent_indices[limb[_LIMB_DATA_IDX_EFFECTOR_INDICES][0]]
             for limb in self.ik_limb_data], dtype=wp.int32)
        self.two_bone_ik_hint_references = wp.array(
            [self.effector_mapped_indices[limb[_LIMB_DATA_IDX_HINT_REF]]
             for limb in self.ik_limb_data], dtype=wp.int32)
        self.two_bone_ik_hint_offsets = wp.array(
            [limb[_LIMB_DATA_IDX_HINT_OFFSET] for limb in self.ik_limb_data],
            dtype=wp.vec3)
