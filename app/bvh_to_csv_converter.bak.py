...
class Viewer:
    def __init__(self, viewer, config):
        ...
        self.retarget_target_options = ['unitree_g1', 'gr1t2']
        self.retarget_solver_options = ['Newton']
        self.retarget_solver_idx     = 0
        target_name = self.config.get('retarget_target', 'unitree_g1')
        self.retarget_target_idx     = self.retarget_target_options.index(target_name)
    ...
    def load_csv_file(self, path):
        self.robot_csv_animation_buffers[0] = csv_utils.load_csv(
            path, csv_config=self.csv_config)
