...
from soma_retargeter.targets import get_target, target_names
...
class Viewer:
    def __init__(self, viewer, config):
        ...
        self.target = get_target(self.config['retarget_target'])
        robot_builder = self.target.build_model_builder()
        self.robot_csv_config = csv_utils.get_csv_config(self.target.name)
    ...
    def load_csv_file(self, path):
        self.robot_csv_animation_buffers[0] = csv_utils.load_csv(
            path, csv_config=self.robot_csv_config)
    ...
    def update_robot_states(self):
        for i in range(self.num_robots):
            ...
            if self.robot_csv_animation_buffers[i] is not None:
                ...
                wp.copy(
                    self.model.joint_q, wp.array(data, dtype=wp.float32),
                ...
                root_tx = wp.mul(
                    robot_offset,
                    wp.transform(
                        *self.robot_default_joint_q_values[
                            joint_q_offset:(joint_q_offset + 7)]))

                wp.copy(
                    self.model.joint_q,
                    wp.array(
                        self.robot_default_joint_q_values[
                            joint_q_offset:(joint_q_offset + self.robot_num_joint_q)],
                        dtype=wp.float32),
                    joint_q_offset,
